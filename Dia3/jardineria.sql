...
select ciudad,telefono from oficina where pais = 'España'; -- Devuelve un listado con la ciudad y el teléfono de las oficinas de España.
...
select distinct codigo_cliente from pago where year(fecha_pago) = 2008; -- Utilizando la función YEAR de MySQL
select distinct codigo_cliente from pago where date_format(fecha_pago, '%Y') = '2008'; -- Utilizando la función DATE_FORMAT de MySQL.
select distinct codigo_cliente from pago where fecha_pago >='2008-01-01' and fecha_pago < '2009-01-01'; -- Sin utilizar ninguna de las funciones anteriores.
